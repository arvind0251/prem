...
from IQ_Paid.utils.database import get_assistant
from IQ_Paid.button_styles import danger_button, success_button
...
def welcomepic(pic, user, chatname, id, uname, brightness_factor=1.3):
    background = Image.open("IQ_Paid/assets/nand.png").convert("RGBA")
...
async def welcome_cmd(_, message: Message):
    ...
    btn = InlineKeyboardMarkup([
        [
            success_button(text="ᴇɴᴀʙʟᴇ", callback_data=f"wlc_on_{chat_id}"),
            danger_button(text="ᴅɪꜱᴀʙʟᴇ", callback_data=f"wlc_off_{chat_id}")
        ]
    ])
...
async def greet_new_member(_, member: ChatMemberUpdated):
    ...
    if member.new_chat_member and not member.old_chat_member and member.new_chat_member.status != "kicked":
        ...
        msg = await app.send_photo(
            ...
            photo=welcomeimg,
            caption=f"""
**⏤͟͟͞͞★ ʜᴇʟʟᴏ ᴅᴇᴀʀ ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ : {member.chat.title}**
